	...
	t.Run("picks up members added in-process and overlays online status", func(t *testing.T) {
		...
		repoMock.On("GuildByRealmAndID", mock.Anything, realmID, guildID).Return(&repo.Guild{
			...
			GuildMembers: []*repo.GuildMember{
				...
				{PlayerGUID: athenaGUID, GuildID: guildID, Status: repo.GuildMemberStatusOffline, LogoutTime: 30},
			},
		}, nil)
		...
		assert.NoError(t, cache.HandleCharacterLoggedIn(events.GWEventCharacterLoggedInPayload{RealmID: realmID, CharGUID: athenaGUID}))
		...
		assert.Equal(t, repo.GuildMemberStatusOnline, statuses[athenaGUID])
		assert.Equal(t, repo.GuildMemberStatusOffline, statuses[cutsGUID])

		assert.NotNil(t, cache.guildMembersCache[realmID][cutsGUID])
		assert.NotNil(t, cache.guildMembersCache[realmID][athenaGUID])
	})
